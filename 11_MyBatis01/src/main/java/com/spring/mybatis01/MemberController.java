...
public class MemberController {
	...
	@RequestMapping("member_contetn.do")
	...
	public void modifyOk(MemberDTO dto,HttpServletResponse response,@RequestParam("member_pwd") String member_pwd) throws IOException {
		
		response.setContentType("text/html; charset=UTF-8");
		PrintWriter out = response.getWriter();
		
		if(member_pwd.equals(dto.getPwd())) {
			int check = this.dao.updateMember(dto);
			if(check>0) {
				out.println("<script>");
				out.println("alert('회원 수정 성공!!')");
				out.println("location.href='member_contetn.do?num="+dto.getNum()+"'");
				out.println("</script>");
			}else {
				out.println("<script>");
